...
| 17 | [ ] | LOW | `docs/` | No migration guide exists any more. `docs/llm-filesystem-migration.md` was deleted for v1.0.0 because roughly half of it documented fields that have never existed in this codebase, and it claimed 100% API compatibility directly above its own Breaking Changes section. Anyone coming from `fast-filesystem-mcp` now has only `--help` and `docs/llm-filesystem-commands.md`, which are true but say nothing about what changed. If migration demand appears, write a short replacement from behaviour verified against the built binary rather than restoring the old file. |
