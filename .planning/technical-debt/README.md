...
| 16 | [ ] | INFO | `commands/search.go` | `search-code --format json` costs slightly **more** in minimal mode than with `--full`: minimal and full carry the same match fields in every invocation (`content`, `file`, `line`, plus `context` in both modes when `--context N` asks for it), so the projection removes nothing while minimal still adds the `next_steps` payload - 149 bytes in the benchmarked invocation, smaller with `--context`, where the hint naming it is suppressed - that `--full` omits to preserve AC12. Not a defect - fixing it means either dropping contextual disclosure from minimal JSON or breaking the legacy guarantee. **Corrected twice:** this entry first claimed the projection could never remove anything for this command, which was only true because `--context N` was being silently discarded; once that bug was fixed it then claimed `--context` makes the minimal set genuinely smaller than full, which is false of the shipped code - the fix made the minimal projection *keep* `context`, so minimal JSON remains slightly larger than full whether or not `--context` is given, for the same `next_steps` reason. Found by `benchmarks/tokens.sh`; explained at the foot of `benchmarks/results-tokens.md`. |
| 17 | [ ] | LOW | `docs/` | No migration guide exists any more. `docs/llm-filesystem-migration.md` was deleted for v1.0.0 because roughly half of it documented fields that have never existed in this codebase, and it claimed 100% API compatibility directly above its own Breaking Changes section. Anyone coming from `fast-filesystem-mcp` now has only `--help` and `docs/llm-filesystem-commands.md`, which say nothing about what changed; the same invented-field claims were removed from that file's API Parity section in review, so both are now accurate. If migration demand appears, write a short replacement from behaviour verified against the built binary rather than restoring the old file. |
